...
//Add for support asynframe framework
#include "IAsynFrame_i.c"
static InstancesManager *_g_lpInstancesManager = 0;

QT_BEGIN_NAMESPACE

void frame_SetInstancesManager(/* [in] */InstancesManager *lpInstancesManager)
{
...
{
    //Add for support asynframe framework
    if (m_lpThreadMessagePump)
        m_lpThreadMessagePump->Release();
    //end Add
}
...
bool QEventDispatcherWin32::processEvents(QEventLoop::ProcessEventsFlags flags)
{
    ...
    //Add for support asynframe framework
    if (_g_lpInstancesManager &&
        m_lpThreadMessagePump == 0) {
        _g_lpInstancesManager->NewInstance(0, 2/*TC_Windows*/, IID_IThreadMessagePump, (IUnknown**)&m_lpThreadMessagePump);
    }
    //end Add
        ...
        while (!d->interrupt.loadRelaxed()) {
            ...
            if (!(flags & QEventLoop::ExcludeUserInputEvents) && !d->queuedUserInputEvents.isEmpty()) {
                ...
                if (m_lpThreadMessagePump == 0) {
                    ...
                    FRAMEMSG tmp;
                    HRESULT ret = m_lpThreadMessagePump->WaitMessage(0, 0, &tmp);
                    if (ret == E_ABORT ) {
                    ...
                    if (ret == S_FALSE ) {
                        ...
                        if (tmp.type != 2) {
                            m_lpThreadMessagePump->PumpMessage(0);
                            continue;
                        } else {
                            memcpy(&msg, tmp.data, sizeof(MSG));
                        }
                    }
                }
                //end Add
